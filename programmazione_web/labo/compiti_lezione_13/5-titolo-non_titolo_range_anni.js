...
app.get('/movies', async(req, res) => {
  ...
  const non_titolo = req.query.excl;
  ...
  if(!non_titolo) {
    return res.status(400).json({errore: 'inserire un titolo da escludere'});
  }
...
connectDB()
  .then(() => {
    app.listen(PORT, () => {
      ...
      console.log(`Link modello: http://localhost:${PORT}/movies?titolo=<titolo>$excl=<escludere>&min=<min>&max=<max>`)
    });
  })
  .catch(err => {
