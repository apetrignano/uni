...
Json ha pochi tipi di dato, sono 8, e la loro descrizione è presente a pagina 69 in `06 XML_JSON.pdf`.


# Lezione 8

In questa lezione, abbastanza pratica perciò con pochi appunti, il profe sta iniziando a spiegare `rest`.

Per ora stiamo guardado swapi, quello che si evince è che è importante il cosiddetto `uri`, tramite questo si può risalire alla struttura dati che ci serve; per ricevere il dato è evidente che serve una `get`, qualsiasi cosa essa voglia dire; quindi, si fa una richiesta a una cosa che ancora non conosciamo, chiamato *backend*, esso ci da un dato `json` ed esso viene "convertito" dove vogliamo usarlo in `javascript`.

Per fare richiesta di dati serve una `API key`, 

Di seguito viene mostrata la sintassi di più o meno ogni richiesta che si vede negli url delle pagine web, per ogni valore.

```
url?parametro1=valore&parametro2=valore&parametro3=valore
```
...
Viene dato errore come risultato, il modo corretto sarebbe:

```
https://api.themoviedb.org/3/movie/popular?language=en-US&page=1&api_key=sdjeofh480r4tfghw
```
La key è scritta a caso.

Poi non è che facciamo coincidere il nostro backend con quello di tmdb in questo caso, è un'*integrazione* del nostro, poi ci saranno altre funzioni che non condivideremo con tmbd ad esempio.

## Promise e fetch

Si parla di ciò quando si metttono in relazione il codice *produttore* e *consumatore*, quindi semplicemente a noi interessa quando facciamo una richiesta al nostro bellissimo tmdb; di seguito lo header della richiesta:
...
L'oggetto creato ha delle proprietà: `state` e `result`.

Invece `fetch` è un metodo, è il più moderno e versatile.
...
- senza alcuna option la richiesta diventa una semplice `get`.

Ottenere una risposta consta di due fasi: *valutazione dello stato* e *risposta*.

### Valutazione dello stato

`HTTP` ha uno status, con delle proprietà, l'ok è un numero tra 200 e 299, si parte dal 100 (famiglia dei 100) e si arriva al 599.
...
let response = await fetch(url);
  if (response.ok) { // se l'HTTP-status è 200-299
    var json = await response.json(); // così si ha conversione da json (illegibile) ad una variabile javascript (leggibile)
  } else {
  alert("HTTP-Error: " + response.status);
}
```
 - `await` serve perchè quando ci sono più processi in contemporanea ogni istruzione "attende".

#### Per i nostri progetti

Di base semplicemente "si toglie" la parte dei contenuti, che viene gestita da `<script>`; quindi nella struttura si tolgono le scritte dai div banalmente.

Prossimi steps:

- chiedere i dati con fetch;
- gestire la risposta e convertire json in js;
- estrarre dati da variabile;
- modificare 
